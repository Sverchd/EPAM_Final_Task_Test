...
namespace BusinessLogicLayer.Services
{
    public class CourseService : ICourseService
    {
        ...
        public List<Course> GetCoursesByTheme(Theme theme)
        {
            ...
            //ar entityCourses = _facultyDbContext.Courses
             //   .Include(x => x.theme)
        ...
        public List<Course> GetCoursesByStudent(string email)
        {
            ...
            //var selectedCourses = courses.Where(x => x.students.Contains(students);
            courses.ForEach(x => x.length = (x.end - x.start).Days);
            return selectedCourses;
        }
        public int Register(int CourseId, string username)
        {
            if (_courseRepository.GetCourseById(CourseId).students.Where(s => s.Name == username).Any())
            {
                return 1;
            }
            else
